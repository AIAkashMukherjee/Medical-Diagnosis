...
from .query import get_diagnosis
...
async def diagnos(user=Depends(authenticate),doc_id:str=Form(...),question:str=Form(default="Please provide a diagnosis based on my report")):
    ...
    if user['role']=='patient' and report['uploader']!=user['username']:
        raise HTTPException(status_code=406,detail="Not authorized to access this report")
    
    # if user is a patient and want diagnosis from his own report
    if user['role']=='patient':
        result= await get_diagnosis(query=question,username=user['username'],doc_id=doc_id)
        # persist the diagnosis report (store report in mongodb)
        diagnosis_collection.insert_one({
            "doc_id": doc_id,
            "requester": user["username"],
            "question": question,
            "answer": result.get("diagnosis"),
    ...
    if user["role"] in ("doctor","admin"):
        raise HTTPException(status_code=407,detail="Doctors and admins cannot access for diagnosis with this endpoint")
...
def get_patient_diagnosis(patient_name: str, user=Depends(authenticate)):
